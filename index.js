...
client.on("message", async message => {
  ...
  if(message.channel.type === "dm"){
	...
	client.channels.cache.get(config.log).send({embed: newTicket})
      ...
      const newChannel = new Discord.MessageEmbed()
        ...
        .setTimestamp()
      await client.channels.cache.get(channel.id).send(/*"<@&755583136712097845>",*/{embed:newChannel});
    ...
    if(message.attachments.size > 0){
      ...
      if (message.content.startsWith(`${config.prefix}requestmod`)) {
        message.react("✅")

        client.channels.cache.get(active.channelID).send("<@&748937666497609738> <@&748938240571998310>")
      }
    }
  ...
  if(support){
    ...
    if(message.content.startsWith(`${config.prefix}reply`)){
      ...
      if(message.member.roles.cache.has("831192749629505557")){
        prefix[message.member.id] = "[Staff] "
      }

      if(message.member.roles.cache.has("755583136712097845")){
        prefix[message.member.id] = "[Support] "
      } 

      if(message.member.roles.cache.has("748937666497609738")) {
        prefix[message.member.id] = "[Moderator] "
      }

      if(message.member.roles.cache.has("748938240571998310")){
        prefix[message.member.id] = "[Administrator] "
      }
      ...
      if(message.attachments.size > 0){
        ...
        return supportUser.send(`${prefix[message.member.id]}${message.author.username} > ${msg}`);
      }
    };
    ...
    if(message.content.toLowerCase() === `${config.prefix}complete`){
      ...
      async function end(userID){
        ...
        let serverMember = client.users.cache.get(support.targetID)
        let emb = new Discord.MessageEmbed()
        .setColor("YELLOW")
        .setAuthor(`${serverMember.username}#${serverMember.discriminator}`)
        .setDescription(`🔒 **| Your ticket, #${actualticket} has been closed! If you need anything else, DM me!**`)
        .setFooter(`[Closed by: ${message.author.username}#${message.author.discriminator}]`)

        return client.users.cache.get(support.targetID).send({embed: emb})
      }
    };
})
      ...
      client.users.fetch(`${args[0]}`).then(async user => {
        ...
        if(data === true){
          await table.delete(`isBlocked${args[0]}`);
                ...
                .setDescription(`<a:check_ravena:831209699806281798> **User has been unblocked!**`)
                .setColor("GREEN")
                .setAuthor(user.tag)
                
                client.channels.cache.get(config.log).send({embed: emb})
